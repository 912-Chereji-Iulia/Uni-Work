...
class Undirected_Graph:
    ...
    def powerset(self, nodes):
        l = list(nodes)
        return chain.from_iterable(combinations(l, n) for n in range(0, len(l)))

    def cover_vertex(self):
        nodes = self.parse_vertices()
        result = []
        nr_min_nodes = 100000000
        powerset = self.powerset(nodes)
        for set in powerset:
            ok = 1
            dict_edges = self.cost
            for j in dict_edges.keys():
                dict_edges[j] = True
            for j in set:
                ...
                for k in self.vertices[j]:
                    dict_edges[(j, k)] = False 
                    dict_edges[(k, j)] = False
            for j in dict_edges.keys():
                if dict_edges[j] == True:
                    ok = 0
                    break
            if ok == 1:
                if len(set) < nr_min_nodes:
                    nr_min_nodes = len(set)
                    result = set
                    break
        return [result, nr_min_nodes]

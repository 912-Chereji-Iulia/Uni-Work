...
class ActivityRepo:
    def __init__(self, activity_list,person_repo:PersonRepo):
    ...
    def add_activity(self, act: Activity):
        ...
        ok=True
        for i in range(len(act.person_ids)):
            if self._persons.person_in_list(act.person_ids[i]) == False:
    ...
    def update_activity(self,act_id,person_ids,date,start_time,end_time,desc):
        ...
        for p in self._activities:
            if act_id == p.activity_id:
                ok1 = True
                for i in person_ids:
                    if self._persons.person_in_list(i) == False:
                        raise ActivityRepoExceptions("Person with this id is not in the planner.")
                if ok1:
